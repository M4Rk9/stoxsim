...
| M7 — Release and capacity evidence | Current production verification and documented operating envelope | Final-candidate DAST and learner acceptance; isolated backup restore; bounded load test on disposable target; latency/error/resource evidence; completed sign-off; release notes and version aligned with deployed SHA | Candidate evidence automation implemented for review: bounded authenticated load, resource samples and isolated restore comparison. Owner reports deployed feature acceptance; production DAST and uptime runs verified. Dedicated recovery and VPS load workflows are implemented; their real executions and final release sign-off remain open; see [evidence contract](M7_RELEASE_EVIDENCE.md). |
