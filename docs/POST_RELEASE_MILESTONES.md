...
| M5 — Advanced portfolio history | Portfolio snapshots, benchmark comparison and historical risk views | Document available licensed data and retention; handle cash flows, gaps, currencies and limited history correctly; enforce premium capabilities on backend; deterministic numerical fixtures | Implemented for review: nightly account observations, gated NIFTY/SPY comparisons, cash/gap-aware metrics and plan enforcement. CI/deployment acceptance pending; benchmark retention permission must be reviewed before activation. See [methodology and acceptance](PORTFOLIO_HISTORY.md). |
