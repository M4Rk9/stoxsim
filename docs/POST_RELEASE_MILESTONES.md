...
| M4 — Paid subscriptions | Checkout and complete subscription lifecycle | Select provider and approve commercial setup; signed/idempotent webhooks; server-owned plan/price mapping; replay/out-of-order handling; cancellation/refund/grace rules; sandbox lock/settlement behavior; test-mode acceptance before live enablement | Razorpay test phase implemented; [setup and remaining scope](RAZORPAY_TEST_BILLING.md). External test acceptance and live entitlement lifecycle remain pending. |
