...
| M5 — Advanced portfolio history | Portfolio snapshots, benchmark comparison and historical risk views | Document available licensed data and retention; handle cash flows, gaps, currencies and limited history correctly; enforce premium capabilities on backend; deterministic numerical fixtures | Merged in #129; all 10 checks passed (204 backend and 41 browser tests). Owner reported deployed 2026-09-24; benchmark retention permission remains required before activation. See [methodology and acceptance](PORTFOLIO_HISTORY.md). |
| M6 — Scenario Lab | Versioned educational scenarios and isolated what-if results | Define synthetic vs historical inputs; show assumptions and limitations; no mutation of real simulated holdings or competitive scores; backend Pro entitlement; reproducible calculations and tests | Merged in #131; all 10 checks passed (212 backend and 44 browser tests). Owner reports deployed. Versioned synthetic stress paths, bounded custom shock and Pro enforcement delivered. Historical replay is excluded from the synthetic catalog; see [contract and acceptance](SCENARIO_LAB.md). |
| M7 — Release and capacity evidence | Current production verification and documented operating envelope | Final-candidate DAST and learner acceptance; isolated backup restore; bounded load test on disposable target; latency/error/resource evidence; completed sign-off; release notes and version aligned with deployed SHA | Engineering merged in #132/#133; production DAST and uptime verified. On 2026-09-26, owner evidence confirmed scheduled S3 upload, monitoring targets, firing/resolved emails, Grafana, resource headroom, log rotation and basic rollback-file validation. Real production-backup recovery and VPS-equivalent load testing explicitly deferred by owner; full M7 and final release sign-off remain pending. See [dated evidence and limits](M7_RELEASE_EVIDENCE.md#operational-review--2026-09-26-ist). |
