...
| M4 — Paid subscriptions | Checkout and complete subscription lifecycle | Select provider and approve commercial setup; signed/idempotent webhooks; server-owned plan/price mapping; replay/out-of-order handling; cancellation/refund/grace rules; sandbox lock/settlement behavior; test-mode acceptance before live enablement | Owner confirmed test checkout/cancellation on 2026-09-23. Opt-in admin test benefits, renewal/grace reconciliation and sandbox order locking implemented; [setup and remaining scope](RAZORPAY_TEST_BILLING.md). Lifecycle deployment acceptance, refunds, scheduled cancellation and live approval remain pending. |
